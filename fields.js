...
import {
  ...
  ENCADREMENT_TYPES, ENCADREMENT_STATUTS, PRESTATION_TYPES, PRESTATION_STATUTS,
  RAYONNEMENT_CATEGORIES, PROJET_STATUTS, PROJET_ROLES, FINANCEMENT_TYPES,
  AFFILIE_STATUTS,
} from './constants.js'
...
export const IDENTIFICATION_FIELDS = [
  ...
  { id: 'axe_secondaire', label: 'Axe secondaire',  type: 'select', required: false, options: ['Aucun', ...GSMI_AXES] },
  ...
  { id: 'expertise_domaines', label: "Domaines d'expertise", type: 'textarea', required: false,
...
export const PUBLICATION_TABLE = {
  ...
  hint: 'Saisir le DOI puis cliquer "Vérifier" pour un remplissage automatique. Une publication n\'existe qu\'une fois dans la base (le DOI est l\'identifiant unique) — si un collègue GSMI est co-auteur, il ne doit pas la ressaisir.',
  cols: [
    ...
    { id: 'impact_factor', label: 'Impact Factor',      type: 'number', w: 100, required: false, min: 0, hint: 'Saisie manuelle (Scopus/JCR)' },
...
export const ENSEIGNEMENT_TABLE = {
  ...
  cols: [
    { id: 'module',   label: 'Module',           type: 'text',   w: 200, required: true },
    { id: 'filiere',  label: 'Filière',           type: 'text',   w: 160, required: false },
...
export const ENCADREMENT_TABLE = {
  ...
  aggregates: [
    ...
    { k: 'Masters',    fn: r => r.filter(x=>x.type==='Master').length },
...
export const PROJET_FIELDS = [
  { id: 'num_as',    label: 'N° AS',               type: 'text',   required: true, validate: req,
    hint: 'Identifiant unique du projet — utilisé pour éviter les doublons' },
  ...
  { id: 'co_responsables', label: 'Co-responsables', type: 'textarea', required: false, hint: 'Un email par ligne' },
...
export const PROJET_LIVRABLE_TABLE = {
  ...
  cols: [
    { id: 'type',      label: 'Type',     type: 'select', w: 140, required: true,
      options: ['Publication','Thèse','Master','Rapport','Brevet','Prestation'] },
...
export const AFFILIE_IDENTIFICATION_FIELDS = [
  ...
  { id: 'taches_contrat', label: 'Tâches convenues dans le contrat', type: 'textarea', required: true, validate: req },
]
...
export const PREVISION_FIELDS = [
  ...
  { section: 'Formation', fields: [
    ...
    { id: 'masters',          label: 'Masters à encadrer',       type: 'number', required: false, min: 0 },
