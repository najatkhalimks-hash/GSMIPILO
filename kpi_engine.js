...
import {
  ...
  getRayonnementsByPerson, getEnseignementsByPerson, fractionalCount,
} from './db.js'
...
export function computeRealisations(email, annee) {
  ...
  return {
    ...
    formation: {
      doctorants: doctorants.length, masters: masters.length, pfe: pfe.length,
      doctorantsDiplomes, mastersDiplomes, hEnseignement,
    },
...
export function buildComparatif(prevision, derniereRevision, realisations) {
  ...
  const rows = [
    ...
    { label: 'Masters à encadrer',       A: A.masters,         B: B.masters,         C: R.formation.masters },
